...
import { MyCacheModule } from '../../cache.module';
...
import { Reflector, type DiscoveryService, type MetadataScanner } from '@nestjs/core';
import { NoCache } from '../../decorator/noCache.decorator';
...
describe('缓存拦截器集成测试', () => {
  ...
  beforeEach(async () => {
    ...
    cacheMonitorService = module.get<CacheMonitorService>(CacheMonitorService);
  ...
  describe('测试全部DI是否正常', () => {
    ...
    it('cacheInterceptor所需求的DI是否正常', () => {
      const cacheInterceptorGlobalInstance = app.get<CacheInterceptor>(CacheInterceptor);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
      const httpAdapterHost = (cacheInterceptorGlobalInstance as any).httpAdapterHost;
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
      const cacheMonitorService = (cacheInterceptorGlobalInstance as any).cacheMonitorService;
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
    ...
    it('cacheTokensService所需求的DI是否正常', () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      const discoveryService = (cacheTokensService as any).discorveryService as DiscoveryService;
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      const metadataScanner = (cacheTokensService as any).metadataScanner as MetadataScanner;
  ...
  describe('测试缓存拦截器测试', () => {
    it('缓存拦截器是否正常工作', async () => {
      ...
      expect(secondResponse.text).toBe('你好啊，缓存测试，这里是缓存测试的结果');
    ...
    it('缓存过期是否正常', async () => {
      const server: Server = app.getHttpServer() as unknown as Server;
      const firstResponse = await request(server).get('/test/expire-3s').expect(200);
      expect(firstResponse.text).toBe('你好啊，这里是3秒过期的缓存测试');
      expect(firstResponse.headers['x-cache']).toBe('MISS');

      const secondResponse = await request(server).get('/test/expire-3s').expect(200);
      ...
      const thirdResponse = await request(server).get('/test/expire-3s').expect(200);
