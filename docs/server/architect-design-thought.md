...
export class Container {
    ...
    async registerProvider(provider: any): Promise<void> {
        ...
        if (!isInjectable && !isController) {
            throw new Error(`${providerClass.name} must be marked with @Injectable() or @Controller()`);
        }
...
export class NestApplication {
    ...
    private validateRootModule(): void {
        ...
        if (!isModule) {
            throw new Error(`${this.rootModule.name} is not a valid module. Did you forget to add @Module() decorator?`);
        }
    }
...
export class RouteScanner {
    ...
    async scanController(controllerClass: Function): Promise<void> {
        ...
        for (const methodName of methodNames) {
            ...
            if (httpMethod) {
                ...
                console.log(`📍 Registered route: ${httpMethod} ${fullPath} -> ${controllerClass.name}.${methodName}`);
            }
        }
    }
    ...
    private buildPath(controllerPath: string, methodPath: string): string {
        ...
        return ('/' + [cleanControllerPath, cleanMethodPath].filter(Boolean).join('/')).replace(/\/+/g, '/');
    }
    ...
    findRoute(method: string, url: string): RouteInfo | null {
        ...
        if (route) {
            console.log(`🎯 Route matched: ${method} ${url} -> ${route.controllerClass.name}.${route.methodName}`);
        }
    ...
    printRoutes(): void {
        ...
        this.routes.forEach(route => {
            console.log(`  ${route.method.padEnd(6)} ${route.path.padEnd(20)} -> ${route.controllerClass.name}.${route.methodName}`);
        });
        console.log();
    }
}
